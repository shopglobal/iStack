...
contract StakeToken_DeFi is _MSG, ERC20, Auth, ISTAKE {
    ...
    function RewardsSupply(uint256 _poolId)
        public
        view
        returns (uint256)
    {
        // return IERC20(REWARDS_TOKEN[_poolId]).balanceOf(_wallet);
        return SupplyCap[_poolId]-IERC20(REWARDS_TOKEN[_poolId]).totalSupply();
    }
        ...
        returns (bool)
    {
        ...
        require(MaxSupply[_poolId] == false, "Revert: check supply");
        ...
        (bool successA) = transferFrom(_msgSender(), address(this), pool_fee);
        // bool successB = transfer(STAKE_POOL, pool_fee);
        (bool successB) = transferFrom(_msgSender(), STAKE_POOL, tokenAmount);
        require(
        ...
        // );
        (bool successC) = ISTAKEPOOL(STAKE_POOL).Stake_Tokens(
                tokenAmount,
                payable(_msgSender()),
                _poolId
            );
        ...
        iStack_Core[_poolId].totalTokenStaked = TotalTokenStaked(_poolId).add(
            tokenAmount
        );
        ...
        returns (bool)
    {
        ...
        require(
            uint256(amountToken) <= uint256(user.stack.totalStaked),
            "Revert: amount stake mismatch"
        );
        // todo
        user.stack.totalStaked = user.stack.totalStaked.sub(amountToken);
        if (uint256(msg.value) >= uint256(0)) {
            OPERATOR.transfer(msg.value);
            iStack_Core[_poolId].totalEtherFees = iStack_Core[_poolId]
                .totalEtherFees
        ...
        (bool successA) = ISTAKEPOOL(STAKE_POOL).UnStake_Tokens(
                amountToken,
                payable(_msgSender()),
                _poolId
            );
        ...
        uint amountBurn = (amountToken * burnRate) / BP;
        (bool successB) = ERC20._burn(_msgSender(), amountBurn);
        require(
            successB == true,
            "Revert: burn err"
        ); 
        ...
        returns (bool)
    {
        ...
        if (
            uint256(IERC20(REWARDS_TOKEN[_poolId]).totalSupply()) +
                uint256(pendingRewards) >
            uint256(SupplyCap[_poolId]) ||
    ...
    // function ClaimNetworkRewards(uint256 _poolId)
    //     public
    //     authorized
    //     returns (bool)
    // {
    //     return Network_Claim_Rewards(_poolId);
    // }

    // function Network_Claim_Rewards(uint256 _poolId) private returns (bool) {
    //     User storage user = users[address(this)][_poolId];
    //     (uint256 pendingRewards, , , , bool userCanClaim) = checkUserStakes(
    //         address(this),
    //         _poolId
    //     );
    //     require(userCanClaim == true, "Revert: User can not claim");
    //     require(
    //         uint256(pendingRewards) > uint256(0),
    //         "Revert: No pending rewards"
    //     );
    //     require(
    //         block.timestamp >
    //             user.stack.lastStakeTime.add(REBATE_TIME_TO_CLAIM[_poolId]),
    //         "Revert: Claimed recently"
    //     );
    //     require(
    //         uint256(user.stack.totalStaked) > uint256(0),
    //         "Revert: not staking"
