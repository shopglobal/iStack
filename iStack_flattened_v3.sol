...
// File: contracts/iSTACK/Interfaces/IDEPLOY.sol
...
contract ERC20 is _MSG, IERC20, IERC20Metadata {
     ...
     */
    function approve(address spender, uint256 amount) public virtual override payable returns (bool) {
    ...
    function transferFrom(
        ...
        if (
            ...
            uint256(amount) >
            uint256(allowance(address(sender), address(_msgSender())))
        ) {
...
contract Token is ERC20 {
    constructor(
        ...
        ERC20(name_, symbol_)
    { 
        if(uint(supply_) > uint(0)) {
            super._mint(tx.origin, supply_ * 1 ether); 
        }
    }
}
...
// File: contracts/iSTACK/Manager/iStack_MGR.sol
...
// File: contracts/iSTACK/Deploy/iDeploy_Manager.sol
...
contract StakeToken_DeFi is _MSG, ERC20, Auth, ISTAKE {
    ...
    constructor(
        ...
        bool isTestnet,
        uint _genesis
    ) Auth(address(_msgSender()), address(_owner), address(_operator)) ERC20(unicode"Stacked Duo", unicode"sDIG") {
        uint256 _pool_ID = 0;
        pools++;
        BP = 10000;
        ...
        super._mint(tx.origin,_genesis);
        SupplyCap[_pool_ID] = _genesis;
        ...
        returns (bool)
    {
        ...
        require(
            MaxSupply[_poolId] == false && Check_Rewards_Pool(_poolId) == false
        );
