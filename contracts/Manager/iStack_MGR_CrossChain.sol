...
contract StakeToken_CrossChain_DeFi_MGR is _MSG, Auth, ISTAKE_MGR {
    ...
    address payable private FAUCET_TOKEN;
    ...
    function newRewardsPool(address payable[] calldata _iStack, uint _pool_ID) public {
        ...
        require(ISTAKE(STAKE_TOKEN).setRewardsPool(REWARDS_POOL[_pool_ID],_pool_ID));
    ...
    function setRewardsPool(address payable _rewardsPool, uint _poolId) public authorized() {
        require(ISTAKE(STAKE_TOKEN).setRewardsPool(_rewardsPool,_poolId));
    ...
    function approveToken(uint amount, address payable account, address token) public operatorOnly() {
        require(IERC20(address(token)).approve(account,amount));
    }
